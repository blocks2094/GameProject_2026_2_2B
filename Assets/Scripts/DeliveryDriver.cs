...
public class DeliveryDriver : MonoBehaviour
{
    ...
    public class DriverEvents
    {
        ...
        [Header("상태 변화 Event")]
        public UnityEvent<float> OnMernyChanged;
    ...
    {
        // 초기 상태 Event 발생
        driverEvents.OnMernyChanged?.Invoke(currentMoney);
    ...
    void HandleMovement()
    {
        ...
        Vector3 moveDiriection = new Vector3(input.x, 0f, input.y);


        if (moveDiriection.magnitude > 0.1f)
        {
            if (!isMoving)
            {
                StartMoving();
            ...
            // 이동 처리
            moveDiriection = moveDiriection.normalized;
            transform.Translate(moveDiriection * moveSpeed * Time.deltaTime, Space.World);

            // 회전 처리

            if (moveDiriection != Vector3.zero)
            {
                Quaternion targetRotation = Quaternion.LookRotation(moveDiriection);
