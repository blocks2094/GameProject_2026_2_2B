...
public class SimpleDungeon : MonoBehaviour
{
    ...
    private void AssignSpecialRooms()
    {
        ...
        foreach(Room room in rooms.Values)
        {
            ...
            int distance = Mathf.Abs(room.center.x) + Mathf.Abs(room.center.y);
        ...
        foreach(Room room in rooms.Values)
        {
            if (room.type == RoomType.Normal) normalRooms.Add(room);
        }

        int trreasureCount = Mathf.Min(Mathf.Max(1, rooms.Count / 4), normalRooms.Count);

        for(int i = 0; i < trreasureCount; i++)
        {
            int index = Random.Range(0, normalRooms.Count);
            normalRooms[index].type = RoomType.Treasure;
    ...
    private void CreateRoomsAndCorridors()
    {
        ...
        TryAddRoom(Vector2Int.zero, startSize, RoomType.Start);     // 더넞ㄴ의 원점에 시작 방을 생성한다

        int attempts = 0;           // 현재까지 방 배치를 시동한 횟수

        while(rooms.Count < roomCount && attempts++ < maxPlacementAttempts)     // 목표 방 개수에 도달하거나 최대 시도 횟수를 넘을 때까지 반복
        {
            List<Room> existingRooms = new(rooms.Values);           
            Room parent = existingRooms[Random.Range(0, existingRooms.Count)];  // 기존 방 중 랜덤하게 선택하여 새 방의 부모방으로 사용한다

            List<Vector2Int> directions = ShuffledDirections();     // 상화좌우 방향의 검사 순서를 랜덤하게 섞는다

            foreach(Vector2Int direction in directions)             // 랜덤하게 섞인 네 방향을 하나씩 검사
            {
                ...
                if (!TryAddRoom(center, size, RoomType.Normal)) continue;

                CreateCorridor(parent.center, center);
        ...
        if (rooms.Count < roomCount)
            Debug.Log($"방 {roomCount} 개 중 {rooms.Count} 개를 생성" );
    }
    ...
    private static List<Vector2Int> ShuffledDirections()
    {
        ...
        for(int i = result.Count - 1; i > 0; i--)       // 뒤쪽 원소부터 하나씩 랜덤 윛와 교환 하는 Fisher-Yates 방식
        {
            int index = Random.Range(0, i + 1);         // 현재 범위 안에서 교환할 임의의 인덱스를 선택

            (result[i], result[index]) = (result[index], result[i]);    // 두 방향의 위치를 서로 교환
        }
    ...
    {
        // 방 중심으로 방이 시작되는 상대 좌표 계산
        int min = -size / 2;
        int max = min + size;

        // 기본 방 주변 한칸의 여백까지 검사하영 방끼리 바로 붙지 않게 한다
        for(int x = min -1; x <= max; x++)      // X 범위보다 한칸
        {
            for(int y = min - 1; y <= max; y++) // Y 범위 보다 한칸
            {
                if (roomFloors.Contains(center + new Vector2Int(x, y))) return false;

                // 검사하려는 좌표가 기본 방 닥에 포함되어 있으면 방 생성 실패
            }
        }

        Room room = new Room(center, size, type);
        rooms.Add(center, room);    // 방의 중심 좌표를 Key로 사용하여 Dictionary에 방을 추가한다

        for(int x = min; x < max; x++)
        {
            for(int y = min; y < max; y++)
            {
                Vector2Int cell = center + new Vector2Int(x, y);    // 방 중심에 상대 좌표를 더하여 실제 셀 좌료 계산
                floors.Add(cell);                                   // 해당 위치 전체 던전의 바닥 목록 추가
                roomFloors.Add(cell);                               // 해당 위치 방 전용 바닥 목록 추가
            }
        }
    ...
    private void CreateCorridor(Vector2Int start, Vector2Int end)
    {
        ...
        {
            WalkX(ref current, end.y);
            WalkY(ref current, end.x);
        }
    ...
    private void ClearGenerated()
    {
        ...
        if (oldRoot == null) return;                    // 기존에 생성도니 던전이 없다면 종료

        if (Application.isPlaying) Destroy(oldRoot.gameObject);
        else DestroyImmediate(oldRoot.gameObject);      // 에디터 모드라면 즉시 삭제 할 수 있는 DestroyImmediate를 사용한다
    }
